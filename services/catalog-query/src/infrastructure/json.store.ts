...
export class JsonStore {
  ...
  portfolio(category?: string) {
    ...
    const byOrder = <T extends { sortOrder?: number }>(items: T[]) =>
      items.slice().sort((a, b) => Number(a.sortOrder ?? 0) - Number(b.sortOrder ?? 0));
    return {
      ...data,
      experiences: byOrder(data.experiences),
      educations: byOrder(data.educations),
      skills: byOrder(data.skills).map((item) => ({ ...item, group: item.group || "Tools & Concepts" })),
      projects: byOrder(published),
      socialLinks: byOrder(data.socialLinks),
      contacts: byOrder(data.contacts),
